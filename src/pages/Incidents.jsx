  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Share link banner — admin only, shows region-specific link */}
        {isAdmin && (
          <div className="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
            <div>
              <p className="text-sm font-semibold text-nova-navy dark:text-white">Employee Incident Report Form</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Share this link with your staff — no login required
              </p>
            </div>
            <button
              type="button"
              onClick={() => {
                const regionCode = region === 'Kenya' ? 'ke' : 'za'
                const link = `${window.location.origin}/report-incident/${regionCode}`
                if (navigator.share) {
                  navigator.share({ title: 'Nova Pioneer Incident Report', url: link })
                    .catch(() => { navigator.clipboard.writeText(link); toast.success('Link copied to clipboard') })
                } else {
                  navigator.clipboard.writeText(link)
                  toast.success('Link copied to clipboard')
                }
              }}
              className="flex items-center gap-2 px-4 py-2 bg-nova-navy text-white text-sm font-semibold rounded-xl hover:bg-nova-navy/90 transition-colors flex-shrink-0"
            >
